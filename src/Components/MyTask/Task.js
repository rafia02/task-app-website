...
const Task = ({task, handleDelete, handleUpdate, handleComplete}) => {
    ...
    return (
        <div className='shadow-xl flex flex-col relative rounded mb-5 bg-purple-50'>
            ...
            <div className='flex justify-between px-5 mb-5 top-40 bottom-0'>
                <button onClick={()=>handleUpdate(_id)} className='border outline-none text-sm text-white font-bold p-2 bg-violet-500 hover:bg-violet-700 rounded'>Update</button>
                <button onClick={()=>handleComplete(_id)} className='border outline-none text-sm text-white font-bold p-2 bg-violet-500 hover:bg-violet-700 rounded'>Complete</button>
